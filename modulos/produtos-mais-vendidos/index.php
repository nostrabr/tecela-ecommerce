...
function urlProdutoMaisVendidos($nome){    
    if($nome != ''){
...
function altProdutoMaisVendidos($nome){    
    return preg_replace("/&([a-z])[a-z]+;/i", "$1", preg_replace('/(\'|")/', "", preg_replace('/( )+/', ' ', str_replace('-',' ',$nome))));
}

//BUSCA OS IDS DOS 5 PRODUTOS MAIS VENDIDOS
$busca_produtos_mais_vendidos = mysqli_query($conn, "
    SELECT COUNT(cp.id) AS total, cp.id_produto 
    FROM carrinho_produto cp
    LEFT JOIN produto AS p ON cp.id_produto = p.id
    WHERE p.status = 1
    GROUP BY cp.id_produto 
    ORDER BY field(p.estoque,0), total DESC
    LIMIT 5;
");

if(mysqli_num_rows($busca_produtos_mais_vendidos) >= 5){

$sql_produtos = '';
while($produtos_mais_vendidos = mysqli_fetch_array($busca_produtos_mais_vendidos)){
    $sql_produtos .= "p.id = ".$produtos_mais_vendidos['id_produto']." OR ";
}
...
<!--CSS-->
<link rel="stylesheet" href="<?= $loja['site'] ?>modulos/produtos-mais-vendidos/css/style.css">

<!--PRODUTOS-->
<section id="produtos-mais-vendidos">

    <div id="produtos-mais-vendidos-produtos">

        <h2 class="d-none">Produtos mais vendidos</h2>
        
        <div class="titulo-section">
            <span>MAIS VENDIDOS</span>
            <a class="d-none d-md-block" href="<?= $loja['site'] ?>mais-vendidos">Ver todos ></a>
        </div>
        ...
        <?php 
            ...
            while($produto = mysqli_fetch_array($busca_produtos)){ 
                ...
                <div class="col-6 col-md-6 col-xl-15">
                    ...
                    <div class="produto-link" onclick="javascript: window.location.href = '<?= $loja['site'] ?>produto/<?= urlProdutoMaisVendidos($produto['produto_categoria']) ?>/<?= urlProdutoMaisVendidos($produto['produto_nome']) ?>/<?= $produto['produto_id'] ?>'" data-produto-id="<?= $produto['produto_id'] ?>" data-produto-lista="Mais Vendidos" data-produto-lista-id="produtos-mais-vendidos">
        ...
        <div class="btn-ver-todos d-block d-md-none">
            <a href="<?= $loja['site'] ?>mais-vendidos">
                <ul>
                    <li>Ver todos</li>
