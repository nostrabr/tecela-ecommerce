...
if(mb_strlen($identificador_carrinho) == 32){
    ...
    $peso_produtos             = $produto_peso*$produto_quantidade;
    $volume_produtos           = (($produto_largura*$produto_altura*$produto_comprimento)/1000000)*$produto_quantidade;
    ...
    '<urn:cotar soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
        ...
        <valorNF>'.$produto_preco.'</valorNF>
        <quantidade>'.$produto_quantidade.'</quantidade>
        <peso>'.$peso_produtos.'</peso>
        <volume>'.$volume_produtos.'</volume>
    ...
    if($erro == 0){
        ...
        $fretes[] = array(
            'empresa' =>  '',
            'nome'    => 'TW',
            'preco'   => money_format('%(#10n', get_tag($resposta,'totalFrete')[0]),
            'prazo'   => $prazo_final
        );
    }
