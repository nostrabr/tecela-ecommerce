    ...
    if(mysqli_num_rows($busca_carrinho) > 0){
        ...
        if(mysqli_num_rows($busca_produtos_carrinho) > 0 & mysqli_num_rows($busca_cliente) > 0 & mysqli_num_rows($busca_endereco) > 0){
            ...
            if($tem_cupom){
                ...
                $faixas_desconto = mysqli_query($conn, "SELECT * FROM pagamento_faixa_desconto WHERE status = 1 AND tipo = 'BOLETO' AND $valor_total BETWEEN de AND ate ORDER BY porcentagem_desconto, valor_desconto DESC LIMIT 1");
            ...
            if(!empty($pedido_id_asaas)){
                ...
                $url_asaas_qrcode = $url_asaas."payments/".$pedido_id_asaas."/identificationField";
                ...
                $boleto_retorno = json_decode($response_boleto,true);

                //BUSCA OS DADOS PIX QUE O ASAAS GERA AUTOMATICAMENTE QUANDO É BOLETO       
                ...
                if(isset($boleto_retorno) & $qrcode_success){
                    ...
                    $identificador_pagamento_boleto = md5(date('Y-m-d H:i:s').'pix'.$id_pedido.$codigo_pedido);

                    //INSERE NO BANCO A REFERENCIA DO ASAAS
                    mysqli_query($conn, "INSERT INTO pagamento_pagseguro (identificador, id_pedido, codigo, tipo, parcelas, valor_parcela, valor_produtos, valor_desconto, valor_juros, valor_frete, tipo_frete, asaas, asaas_link_fatura, asaas_pix_chave, asaas_pix_imagem, asaas_boleto) VALUES ('$identificador_pagamento_boleto','$id_pedido','$pedido_id_asaas','BOLETO','1','$valor_final','$valor_total','$valor_desconto','0','$valor_frete','$tipo_frete',1,'$pedido_url','$qrcode_chave','$qrcode_imagem','$response_boleto')");
...
if($status_processo == 'SUCESSO'){ 
    ?> 
    <form id="form-pagamento-boleto-asaas" style="display: none;" action="../../../envio-email/index.php" method="POST">
        <input type="hidden" name="tipo-envio" value="formulario-pagamento-boleto-asaas">
        <input type="hidden" name="pedido" value="<?= $codigo_pedido ?>">
        <input type="hidden" name="asaas_boleto" value="<?= $response_boleto ?>">
...
//SE GEROU ERRO, VAI DIRETO PARA A CONFIRMAÇÃO EXIBIR O PROBLEMA
    ...
    ?>
    <form id="form-pagamento-boleto-asaas" style="display: none;" action="../../../../carrinho-confirmacao" method="POST">
...
echo "<script>document.getElementById('form-pagamento-boleto-asaas').submit();</script>";