...
$hash_comprador         = trim(strip_tags(filter_input(INPUT_POST, 'hash-comprador-boleto', FILTER_SANITIZE_STRING)));
...
if(!empty($tipo_frete) & !empty($endereco) & !empty($hash_comprador) & !empty($identificador_carrinho) & !empty($identificador_cliente)){
    ...
    if(mysqli_num_rows($busca_carrinho) > 0){
        ...
        if(mysqli_num_rows($busca_produtos_carrinho) > 0 & mysqli_num_rows($busca_cliente) > 0 & mysqli_num_rows($busca_endereco) > 0){
            ...
            //TIPO DE PAGAMENTO
            $pag_seguro['paymentMethod']                    = 'boleto';
            ...
            if($xml->paymentLink != ''){
                ...
                if($xml->error){   
                    ...
                    mysqli_query($conn, "INSERT INTO pagamento_pagseguro (identificador, id_pedido, hash, codigo, tipo, boleto, parcelas, valor_parcela, valor_produtos, valor_desconto, valor_juros, valor_frete, tipo_frete) VALUES ('$identificador_pagamento_pagseguro','$id_pedido','$hash_comprador','$xml->code','BOLETO','$xml->paymentLink','1','$valor_final','$valor_total','$valor_desconto','0','$valor_frete','$tipo_frete')");
                ...
                $_SESSION['pagseguro_boleto_erro'] = 'S';
                ...
                echo "<script>window.location.href = '".$loja['site']."carrinho-frete'</script>";
...
if($status_processo == 'SUCESSO'){ 
    ...
    <form id="form-pagamento-pagseguro" style="display: none;" action="../../../envio-email/index.php" method="POST">
        <input type="hidden" name="tipo-envio" value="formulario-pagamento-pagseguro-boleto">
        <input type="hidden" name="pedido" value="<?= $codigo_pedido ?>">
        <input type="hidden" name="boleto" value="<?= $xml->paymentLink ?>">
