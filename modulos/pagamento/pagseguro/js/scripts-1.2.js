...
$('.carrinho-pagamento-forma-pagamento').click(function(){
    global_valor_compra = $("#valor-compra").val();
});
...
function buscaTokenCartao(){
    ...
    PagSeguroDirectPayment.createCardToken({
        ...
        expirationYear: '20'+global_ano_validade,
        success: function(retorno){
...
function buscaHashComprador(funcao){
    ...
    if(tudo_certo){
        ...
        if(!global_cartao_ok | !global_validade_ok | !global_cvv_ok | $("#cpf").val().length !== 14 | $("#nascimento").val().length !== 10 | $("#nome").val() == '' | $("#parcelas").val() == ''){
            mensagemAviso('erro', 'Preencha os dados do cartão corretamente.', 3000);
        }
        ...
        if($("#nascimento").val().length !== 10){
            $("#nascimento").css("border-color",'firebrick');
        } else {
            $("#nascimento").removeAttr("style");
        }
