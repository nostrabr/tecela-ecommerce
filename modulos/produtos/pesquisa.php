...
//PEGA A PESQUISA 
$pesquisa = trim(strip_tags(filter_input(INPUT_GET, "pesquisa", FILTER_SANITIZE_STRING)));
...
//CADASTRA A PESQUISA PARA O DASHBOARD
$pesquisa_dashboard = str_replace('-',' ',$pesquisa);
mysqli_query($conn, "INSERT INTO busca (busca, visitante) VALUES ('$pesquisa_dashboard','".$_SESSION['visitante']."')");

//SEPARA AS PALAVRAS DA PESQUISA
$pesquisa_aux = explode('-',$pesquisa);

//PREENCHE UM ARRAY COM AS PALAVRAS CHAVE DA PESQUISA QUE POSSUEM MAIS DE DOIS CARACTERES
foreach($pesquisa_aux AS $palavra){
    if(mb_strlen($palavra) > 2){
        $palavras_pesquisa[] = '%'.$palavra;
    } 
}

//BUSCA O TOTAL DE PALAVRAS
$total_palavras_pesquisa = count($palavras_pesquisa);
$sql_pesquisa = 'AND p.nome LIKE "';
$sql_pesquisa_sku = 'OR p.sku LIKE "';
$sql_pesquisa_categoria_nome = 'OR pc.nome LIKE "';

for($i=0; $i < $total_palavras_pesquisa; $i++){
    $sql_pesquisa .= $palavras_pesquisa[$i];
    $sql_pesquisa_sku .= $palavras_pesquisa[$i];
    $sql_pesquisa_categoria_nome .= $palavras_pesquisa[$i];
}
$sql_pesquisa .= '%"';
$sql_pesquisa_sku .= '%"';
$sql_pesquisa_categoria_nome .= '%"';

/*$sql_pesquisa            = 'AND (';
//GERA UMA STRING DE COMPLEMENTO PRO WHERE DA PESQUISA
for($i=0; $i < $total_palavras_pesquisa; $i++){
    $sql_pesquisa .= "p.nome LIKE '".$palavras_pesquisa[$i]."' OR p.palavras_chave LIKE '".$palavras_pesquisa[$i]."'";
    if(($i+1) != $total_palavras_pesquisa){
        $sql_pesquisa .= ' OR ';
    }
}
$sql_pesquisa .= ')';*/
...
//FILTRO DE CATEGORIA
$sql_categorias    = 'WHERE t1.nivel = 1';
$filtro_categoria = 'T';
if(isset($_GET['cat'])){ 
    $filtro_categoria = trim(strip_tags(filter_input(INPUT_GET, "cat", FILTER_SANITIZE_STRING))); 
    if($filtro_categoria != 'T'){
        $sql_categorias   = 'WHERE t1.id = '.$filtro_categoria;
    }
}
...
/***********************************************************************************/
/* BUSCA A HIERARQUIA DAS SUBCATEGORIAS DA CATEGORIA QUE VEIO SELECIONADA, SE VEIO */
/***********************************************************************************/
...
$titulo_pagina                     = 'Pesquisa: '.implode(' ',explode('-',$pesquisa));
...
while($categorias = mysqli_fetch_array($busca_hierarquia)){
    if($categorias['lev1']){ array_push($array_categorias, 'pc.id = '.$categorias['lev1']); }
    if($categorias['lev2']){ array_push($array_categorias, 'pc.id = '.$categorias['lev2']); $array_categorias_filhas[] = array('id' => $categorias['lev2'], 'nome' => $categorias['nome2']); }
    if($categorias['lev3']){ array_push($array_categorias, 'pc.id = '.$categorias['lev3']); $array_categorias_filhas[] = array('id' => $categorias['lev3'], 'nome' => $categorias['nome3']); }
    if($categorias['lev4']){ array_push($array_categorias, 'pc.id = '.$categorias['lev4']); $array_categorias_filhas[] = array('id' => $categorias['lev4'], 'nome' => $categorias['nome4']); }
    if($categorias['lev5']){ array_push($array_categorias, 'pc.id = '.$categorias['lev5']); $array_categorias_filhas[] = array('id' => $categorias['lev5'], 'nome' => $categorias['nome5']); }
    if($categorias['lev6']){ array_push($array_categorias, 'pc.id = '.$categorias['lev6']); $array_categorias_filhas[] = array('id' => $categorias['lev6'], 'nome' => $categorias['nome6']); }
    if($categorias['lev7']){ array_push($array_categorias, 'pc.id = '.$categorias['lev7']); $array_categorias_filhas[] = array('id' => $categorias['lev7'], 'nome' => $categorias['nome7']); }
    if($categorias['lev8']){ array_push($array_categorias, 'pc.id = '.$categorias['lev8']); $array_categorias_filhas[] = array('id' => $categorias['lev8'], 'nome' => $categorias['nome8']); }
    if($categorias['lev9']){ array_push($array_categorias, 'pc.id = '.$categorias['lev9']); $array_categorias_filhas[] = array('id' => $categorias['lev9'], 'nome' => $categorias['nome9']); }
    if($categorias['lev10']){ array_push($array_categorias, 'pc.id = '.$categorias['lev10']); $array_categorias_filhas[] = array('id' => $categorias['lev10'], 'nome' => $categorias['nome10']); }
}

$array_categorias = array_unique($array_categorias);
$sql_categorias   = implode($array_categorias,' OR ').' OR p.id_categoria = 0';
...
//SQL PARA MOSTRAR QUAIS SÃO OS PRODUTOS DE MENOR E MAIOR PRECO
$busca_produtos_valores = mysqli_query($conn,"
    ...
    WHERE p.status = 1 AND ($sql_categorias)
...
$busca_produtos = mysqli_query($conn,"
    ...
    $sql_caracteristicas
    $sql_tags
    WHERE p.status = 1 AND ($sql_categorias) $sql_preco $sql_marcas $sql_genero $sql_idade $sql_pesquisa $sql_pesquisa_categoria_nome $sql_pesquisa_sku
...
<!--PRODUTOS-->
<section id="produtos" class="produtos-pesquisa">
...
<?php if(mysqli_num_rows($busca_produtos) > 0){ ?>
    ...
    <div class="row">
        ...
        <div id="produtos-filtros" class="d-none d-lg-block col-3">
            ...
            <ul>
                ...
                <li class="produtos-caracteristicas">
                    <select id="select-ordenar-por" title="Selecione a forma de ordenação dos produtos listados" onchange="javascript: window.location.href = '<?= $loja['site'].'pesquisa/'.$pesquisa.'/1/'.$filtro_categoria.'/'.$filtro_preco_minimo.'/'.$filtro_preco_maximo.'/'.$filtro_marcas.'/'.$filtro_caracteristicas.'/'.$filtro_genero_idade.'/' ?>'+this.value+'/<?= $filtro_tags ?>';">   
            ...
            <ul>
                ...
                <div class="container-filtro-preco">
                    ...
                    <li class="produtos-caracteristicas">
                        <ul class="produtos-caracteristicas-caixas-selecao">
                            ...
                            <li><div id="slider-range" minimo-geral="<?= $preco_minimo_geral  ?>" minimo-selecionado="<?= $filtro_preco_minimo ?>" maximo-geral="<?= $preco_maximo_geral ?>" maximo-selecionado="<?= $filtro_preco_maximo ?>" url="<?= $loja['site'].'pesquisa/'.$pesquisa.'/1/'.$filtro_categoria ?>" marcas="<?= $filtro_marcas ?>" caracteristicas="<?= $filtro_caracteristicas ?>" genero-idade="<?= $filtro_genero_idade ?>" tags="<?= $filtro_tags ?>"></div></li>
                        </ul>
                    </li> 
                </div>
                ...
                if(count($array_categorias_filhas) > 0){ 
                    ...
                    ?>
                    <li class="produtos-filtros-titulo">Subcategorias</li>
                    <li class="produtos-caracteristicas">
                        <ul class="produtos-caracteristicas-caixas-selecao">
                            <?php foreach($array_categorias_filhas as $k => $v){ 
                                $counts = array_count_values($array_categorias);
                                $total_counts = $counts[$v['id']];
                                if($total_counts > 0){
                                ?>
                                <li class="produto-filtro produtos-caracteristica-caixa-selecao"><a href="<?= $loja['site'].'pesquisa/'.$pesquisa.'/1/'.$v['id'].'/'.$filtro_preco_minimo.'/'.$filtro_preco_maximo.'/'.$filtro_marcas.'/'.$filtro_caracteristicas.'/'.$filtro_genero_idade.'/'.$filtro_ordenacao.'/'.$filtro_tags ?>"><?= $v['nome'] ?><span class="total-resultados-por-filtro">(<?= $total_counts ?>)</span></a></li>
                            <?php } } ?>
                        </ul>
                    </li> 
                <?php } }
                    ...
                    <li class="produtos-caracteristicas">
                        <ul class="produtos-caracteristicas-caixas-selecao">
                            <?php while($marca = mysqli_fetch_array($marcas)){ 
                                ...
                                ?>                    
                                <li class="produto-filtro produtos-caracteristica-caixa-selecao custom-control custom-checkbox">
                                    <input class="custom-control-input" type="checkbox" value="<?= $marca['id'] ?>" name="checkbox-marcas" id="checkbox-marcas-<?= $marca['id'] ?>" url="<?= $loja['site'].'pesquisa/'.$pesquisa.'/1/'.$filtro_categoria.'/'.$filtro_preco_minimo.'/'.$filtro_preco_maximo ?>" caracteristicas="<?= $filtro_caracteristicas ?>" genero-idade="<?= $filtro_genero_idade ?>" tags="<?= $filtro_tags ?>" <?php if(in_array($marca['id'],$array_filtro_marcas)){ echo 'checked'; } ?>>
                                    ...
                                    while($caracteristica = mysqli_fetch_array($caracteristicas)){
                                        ...
                                        if($total_counts > 0){
                                            ...
                                            if($atributo['atributo_visualizacao'] == 'T'){ ?>
                                                <li class="produto-filtro produtos-caracteristica-caixa-selecao custom-control custom-checkbox">
                                                    <input class="custom-control-input" type="checkbox" value="<?= $caracteristica['caracteristica_id'] ?>" name="checkbox-caracteristicas" id="checkbox-caracteristicas-<?= $caracteristica['caracteristica_id'] ?>" genero-idade="<?= $filtro_genero_idade ?>" tags="<?= $filtro_tags ?>" url="<?= $loja['site'].'pesquisa/'.$pesquisa.'/1/'.$filtro_categoria.'/'.$filtro_preco_minimo.'/'.$filtro_preco_maximo.'/'.$filtro_marcas ?>" <?php if(in_array($caracteristica['caracteristica_id'],$array_filtro_caracteristicas)){ echo 'checked'; } ?>>
                        ...
                        <li class="produtos-caracteristicas">
                            <ul class="produtos-caracteristicas-caixas-selecao">
                                ...
                                <?php if($total_counts_kids > 0){ ?>
                                    <li class="produto-filtro produtos-caracteristica-caixa-selecao custom-control custom-checkbox">
                                        <input class="custom-control-input" type="checkbox" value="kids" name="checkbox-idade" id="checkbox-idade-kids" url="<?= $loja['site'].'pesquisa/'.$pesquisa.'/1/'.$filtro_categoria.'/'.$filtro_preco_minimo.'/'.$filtro_preco_maximo.'/'.$filtro_marcas.'/'.$filtro_caracteristicas ?>" <?php if(in_array('kids',$array_filtro_idade)){ echo 'checked'; } ?>>
                                ...
                                <?php if($total_counts_adult > 0){ ?>
                                    <li class="produto-filtro produtos-caracteristica-caixa-selecao custom-control custom-checkbox">
                                        <input class="custom-control-input" type="checkbox" value="adult" name="checkbox-idade" id="checkbox-idade-adult" url="<?= $loja['site'].'pesquisa/'.$pesquisa.'/1/'.$filtro_categoria.'/'.$filtro_preco_minimo.'/'.$filtro_preco_maximo.'/'.$filtro_marcas.'/'.$filtro_caracteristicas ?>" <?php if(in_array('adult',$array_filtro_idade)){ echo 'checked'; } ?>>
        ...
        <div id="produtos-produtos" class="col-12 col-lg-9">
            ...
            <div class="row">
                ...
                while($produto = mysqli_fetch_array($busca_produtos)){ 
                    ...
                    <div id="produto-<?= $produto['produto_id'] ?>" produto="<?= $produto['produto_id'] ?>" class="produto col-12 col-md-6 col-xl-4 col-sxl-4" nome="<?= $produto['produto_nome'] ?>" relevancia="<?= $produto['produto_relevancia'] ?>" categoria="<?= $produto['produto_categoria_id'] ?>" marca="<?= $produto['produto_marca_id'] ?>" atributos="<?= $produto['produto_atributos'] ?>" caracteristicas="<?= $produto['produto_caracteristicas'] ?>" promocao="<?php if($tem_promocao){ echo '1'; } else { echo '0'; } ?>" preco="<?= $produto_preco_venda_busca ?>" estoque="<?php if($em_estoque){ echo '1'; } else { echo '0'; } ?>">
                        ...
                        <div class="produto-link" onclick="javascript: window.location.href = '<?= $loja['site'] ?>produto/<?= urlProduto($produto['produto_categoria']) ?>/<?= urlProduto($produto['produto_nome']) ?>/<?= $produto['produto_id'] ?>'" data-produto-id="<?= $produto['produto_id'] ?>" data-produto-lista="Pesquisa" data-produto-lista-id="produtos-pesquisa">
            ...
            //MONTA A PAGINAÇÃO
            if($total_paginas > 1){ $url_pagina = $loja['site'].'pesquisa/'.$pesquisa; ?>
                ...
                <input type="hidden" id="complemento-pagina" value="<?= '/'.$filtro_categoria.'/'.$filtro_preco_minimo.'/'.$filtro_preco_maximo.'/'.$filtro_marcas.'/'.$filtro_caracteristicas.'/'.$filtro_genero_idade.'/'.$filtro_ordenacao ?>">
