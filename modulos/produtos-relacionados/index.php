...
function urlProdutoRelacionado($nome){
    if($nome != ''){
...
function altProdutoRelacionado($nome){    
    return preg_replace("/&([a-z])[a-z]+;/i", "$1", preg_replace('/(\'|")/', "", preg_replace('/( )+/', ' ', str_replace('-',' ',$nome))));
}

$sql_tags  = 'LEFT JOIN produto_tag AS pt ON pt.id_produto = p.id';
$sql_where = '(p.id_categoria = '.$id_categoria_url;
$busca_tags_produto = mysqli_query($conn, "SELECT id_tag FROM produto_tag WHERE id_produto = '$id_produto_url'");
if(mysqli_num_rows($busca_tags_produto) > 0){
    while($tag_produto = mysqli_fetch_array($busca_tags_produto)){
        $sql_where .= ' OR pt.id_tag = '.$tag_produto['id_tag'];
    }
} else {
    $sql_tags  = '';
}
...
//BUSCA 20 PRODUTOS RELACIONADOS
$busca_produtos = mysqli_query($conn,"
    SELECT DISTINCT(p.id) AS produto_id, p.nome AS produto_nome, p.preco AS produto_preco, p.descricao AS produto_descricao, p.estoque AS produto_estoque, pc.nome AS produto_categoria, 
    ...
    $sql_tags
    WHERE p.status = 1 AND p.id != '$id_produto_url' AND $sql_where    
    ORDER BY field(p.estoque,0), p.relevancia DESC, p.data_cadastro DESC
    LIMIT 20
");
...
<!--CSS-->
<link rel="stylesheet" href="<?= $loja['site'] ?>modulos/produtos-relacionados/css/style.css">

<!--PRODUTOS-->
<section id="produtos-relacionados">

    <div id="produtos-relacionados-produtos">
    
        <h2 class="d-none">Produtos relacionados</h2>

        <div class="titulo-section">RELACIONADOS</div>
        ...
        <div class="row">
            ...
            while($produto = mysqli_fetch_array($busca_produtos)){ 
                ...
                <div class="col-12 col-md-6 col-xl-15">
                    ...
                    <div class="produto-link" onclick="javascript: window.location.href = '<?= $loja['site'] ?>produto/<?= urlProdutoRelacionado($produto['produto_categoria']) ?>/<?= urlProdutoRelacionado($produto['produto_nome']) ?>/<?= $produto['produto_id'] ?>'" data-produto-id="<?= $produto['produto_id'] ?>" data-produto-lista="Relacionados" data-produto-lista-id="produtos-relacionados">
