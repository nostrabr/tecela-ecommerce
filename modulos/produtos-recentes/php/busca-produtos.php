...
if(!empty($produtos) & mb_strlen($visitante) == 32){
    ...
    if(count($array_produtos_recentes) >= 5){
        ...
        function urlProdutoRecente($nome){    
            if($nome != ''){
        ...
        function altProdutoRecente($nome){    
        ...
        if(mysqli_num_rows($busca_produtos) >= 5){
            ...
            while($produto = mysqli_fetch_array($busca_produtos)){ 
                ...
                <div class="col-6 col-md-6 col-xl-15">
                    ...
                    <div class="produto-link" onclick="javascript: window.location.href = '<?= $loja['site'] ?>produto/<?= urlProdutoRecente($produto['produto_categoria']) ?>/<?= urlProdutoRecente($produto['produto_nome']) ?>/<?= $produto['produto_id'] ?>'" data-produto-id="<?= $produto['produto_id'] ?>" data-produto-lista="Recentes" data-produto-lista-id="produtos-recentes">
                        <div class="produto-container produto-container-<?= $contador_produtos ?><?php if(!$em_estoque){ echo ' produto-container-esgotado'; } else { ?> <?php if($tem_promocao){ echo ' produto-container-promocao'; } } ?>">
                            <ul>
                                <li class="produto-container-imagem">
                                    <img src="<?php if($produto['produto_capa'] != ''){ ?><?= $loja['site'] ?>imagens/produtos/media/<?= $produto['produto_capa'] ?><?php } else { ?><?= $loja['site'] ?>imagens/produto_sem_foto.png<?php } ?>" alt="<?= altProdutoRecente($produto['produto_nome']) ?>" title="<?= $produto['produto_nome'] ?>">
                                    <?php if($em_estoque){ ?> 
