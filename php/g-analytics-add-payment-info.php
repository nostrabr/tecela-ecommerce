...
if(mb_strlen($session_visitante) == 32){
    ...
    //PARAMETROS
    $identificador_cupom = filter_input(INPUT_POST,"coupon");
    $tipo_pagamento      = filter_input(INPUT_POST,"payment_type");

    //CORRIGE O NOME DO TIPO DE PAGAMENTO CASO SEJA SOMENTE 'cartao'
    if($tipo_pagamento == "cartao"){ $tipo_pagamento = "Cartão de Crédito"; }
    ...
    $carrinho = mysqli_query($conn, "
        SELECT cp.quantidade AS produto_quantidade, cp.preco AS produto_preco
        FROM carrinho AS c
    ...
    if($n_itens > 0){
        ...
        $retorno = array(
            ...
            "coupon"        => $cupom_nome,
            "payment_type"  => mb_convert_case($tipo_pagamento, MB_CASE_TITLE, 'UTF-8'),
            "items"         => $retorno_itens
        );
