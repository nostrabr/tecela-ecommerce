...
if(isset($_SESSION["DONO"])){
    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        if(!empty($ordem_form) & $nivel_usuario != 'U'){
            ...
            $consulta_banner = mysqli_query($conn, "SELECT imagem, ordem FROM banner_secundario WHERE id = '$id'");
            $banner_banco = mysqli_fetch_array($consulta_banner);

            //ALTERA O LINK
            mysqli_query($conn, "UPDATE banner_secundario SET link = '$link', titulo = '$titulo' WHERE id = '$id'");     

            //SE A IMAGEM FOI, TRATA
            if($banner_banco["imagem"] !== $nome_imagem_form){
                
                //DELETA AS IMAGENS VELHAS
                unlink("../../../../imagens/banners-secundarios/original/".$banner_banco["imagem"]);
                unlink("../../../../imagens/banners-secundarios/pequena/".$banner_banco["imagem"]);
                ...
                mysqli_query($conn, "UPDATE banner_secundario SET imagem = '$nome_imagem' WHERE id = '$id'");            
            ...
            if($banner_banco["ordem"] !== $ordem_form){
                ...
                if($nova_ordem > $ordem_antiga){
                    ...
                    mysqli_query($conn, "UPDATE banner_secundario SET ordem = '$ordem_temporaria' WHERE id = '$id'"); 
                    
                    while($contador < $fim_contagem){
                        
                        $proximo_da_ordem = $contador + 1;
                        
                        $busca_id = mysqli_query($conn, "SELECT id FROM banner_secundario WHERE ordem = '$proximo_da_ordem'");
                        $banner_id = mysqli_fetch_array($busca_id);
                        $id_proximo = $banner_id['id'];
                        
                        mysqli_query($conn, "UPDATE banner_secundario SET ordem = '$contador' WHERE id = '$id_proximo'"); 
                        
                        $contador++;
                        
                    }
                    
                    mysqli_query($conn, "UPDATE banner_secundario SET ordem = '$nova_ordem' WHERE id = '$id'"); 
