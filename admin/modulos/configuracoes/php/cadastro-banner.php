...
if(isset($_SESSION["DONO"])){
    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        if(!empty($ordem) & $nivel_usuario != 'U'){
            ...
            $nome_imagem_mobile = md5(time()."mobile").'.'.$extensao;
            ...
            $identificador = md5(date('Y-m-d H:m:i').$ordem.$nome_imagem);

            //SE A QUE VEIO É MAIOR, ADICIONA
            if($ordem > $qtde_ordens["total_ordem"]){    
                
                //INSERE NO BANCO
                mysqli_query($conn, "INSERT INTO banner (identificador, ordem, imagem_desktop, imagem_mobile, link, cadastrado_por) VALUES ('$identificador','$ordem','$nome_imagem','$nome_imagem_mobile','$link','$identificador_usuario_session')");    
            ...
            //SENÃO, REORDENA    
                ...
                while($velha_ordem >= $ordem){
                    ...
                    $busca_id = mysqli_query($conn, "SELECT id FROM banner WHERE ordem = '$velha_ordem'");
                    $velha_ordem_id = mysqli_fetch_array($busca_id);
                    $id_ordem = $velha_ordem_id["id"];
                    
                    mysqli_query($conn, "UPDATE banner SET ordem = '$nova_ordem' WHERE id = '$id_ordem'");
                ...
                //INSERE NO BANCO
                mysqli_query($conn, "INSERT INTO banner (identificador, ordem, imagem_desktop, imagem_mobile, link, cadastrado_por) VALUES ('$identificador','$ordem_novo_banner','$nome_imagem','$nome_imagem_mobile','$link','$identificador_usuario_session')");
