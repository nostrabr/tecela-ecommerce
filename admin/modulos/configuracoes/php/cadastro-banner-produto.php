    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        if(!empty($ordem) & $nivel_usuario != 'U'){
            ...
            if($ordem > $qtde_ordens["total_ordem"]){    
                ...
                //INSERE NO BANCO
                mysqli_query($conn, "INSERT INTO banner_produto (identificador, ordem, imagem, link, cadastrado_por) VALUES ('$identificador','$ordem','$nome_imagem','$link','$identificador_usuario_session')");    
            ...
            //SENÃO, REORDENA    
                ...
                while($velha_ordem >= $ordem){
                    ...
                    $busca_id = mysqli_query($conn, "SELECT id FROM banner_produto WHERE ordem = '$velha_ordem'");
                    $velha_ordem_id = mysqli_fetch_array($busca_id);
                    $id_ordem = $velha_ordem_id["id"];
                    
                    mysqli_query($conn, "UPDATE banner_produto SET ordem = '$nova_ordem' WHERE id = '$id_ordem'");
                ...
                //INSERE NO BANCO
                mysqli_query($conn, "INSERT INTO banner_produto (identificador, ordem, imagem, link, cadastrado_por) VALUES ('$identificador','$ordem_novo_banner','$nome_imagem','$link','$identificador_usuario_session')");
