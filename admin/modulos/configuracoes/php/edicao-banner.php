...
if(isset($_SESSION["DONO"])){
    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        if(!empty($ordem_form) & $nivel_usuario != 'U'){
            ...
            $nome_imagem_mobile_form = filter_input(INPUT_POST, 'arquivo-mobile', FILTER_SANITIZE_STRING);

            //VERIFICA SE A IMAGEM OU ORDEM FOI ATUALIZADA
            $consulta_banner = mysqli_query($conn, "SELECT imagem_desktop, imagem_mobile, ordem FROM banner WHERE id = '$id'");
            $banner_banco = mysqli_fetch_array($consulta_banner);

            //ALTERA O LINK
            mysqli_query($conn, "UPDATE banner SET link = '$link' WHERE id = '$id'");     

            //SE A IMAGEM FOI, TRATA
            if($banner_banco["imagem_desktop"] !== $nome_imagem_form){
                
                //DELETA AS IMAGENS VELHAS
                unlink("../../../../imagens/banners/original/".$banner_banco["imagem_desktop"]);
                unlink("../../../../imagens/banners/pequena/".$banner_banco["imagem_desktop"]);
                ...
                $diretorio = "../../../../imagens/banners/original/";
                
                //MOVE A IMAGEM PARA O DIRETÓRIO
                move_uploaded_file($_FILES['imagem']['tmp_name'], $diretorio.$nome_imagem);
                
                //GERA UMA MINIATURA
                reduz_imagem("../../../../imagens/banners/original/".$nome_imagem, $nome_imagem, 100, "../../../../imagens/banners/pequena/");
                
                mysqli_query($conn, "UPDATE banner SET imagem_desktop = '$nome_imagem' WHERE id = '$id'");            
                
            } 

            //SE A IMAGEM MOBILE FOI, TRATA
            if($banner_banco["imagem_mobile"] !== $nome_imagem_mobile_form){
                
                //DELETA A IMAGEM VELHA
                unlink("../../../../imagens/banners/original/".$banner_banco["imagem_mobile"]);
                unlink("../../../../imagens/banners/pequena/".$banner_banco["imagem_mobile"]);
                
                //RETIRA A EXTENSÃO DA IMAGEM RECEBIDA
                $extensao_mobile = mb_strtolower(pathinfo($_FILES['imagem-mobile']['name'], PATHINFO_EXTENSION));  
                
                //RENOMEIA
                $nome_imagem_mobile = md5(time()."mobile").'.'.$extensao_mobile;
                
                //DIRETÓRIO DE IMAGENS DE BANNERS
                $diretorio = "../../../../imagens/banners/original/";
                
                //MOVE A IMAGEM PARA O DIRETÓRIO
                move_uploaded_file($_FILES['imagem-mobile']['tmp_name'], $diretorio.$nome_imagem_mobile);
                
                //GERA UMA MINIATURA
                reduz_imagem("../../../../imagens/banners/original/".$nome_imagem_mobile, $nome_imagem_mobile, 100, "../../../../imagens/banners/pequena/");
                
                mysqli_query($conn, "UPDATE banner SET imagem_mobile = '$nome_imagem_mobile' WHERE id = '$id'");            
            ...
            if($banner_banco["ordem"] !== $ordem_form){
                ...
                if($nova_ordem > $ordem_antiga){
                    ...
                    mysqli_query($conn, "UPDATE banner SET ordem = '$ordem_temporaria' WHERE id = '$id'"); 
                    ...
                    mysqli_query($conn, "UPDATE banner SET ordem = '$nova_ordem' WHERE id = '$id'"); 
            ...
            echo "<script>location.href='../../../configuracoes-design-banners.php';</script>";
