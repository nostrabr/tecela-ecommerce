...
if(isset($_SESSION["DONO"])){
    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        $identificador_cupom           = trim(strip_tags(filter_input(INPUT_POST, "identificador", FILTER_SANITIZE_STRING)));
        ...
        if(!empty($identificador_cupom) & !empty($nome) & !empty($quantidade) & !empty($validade) & !empty($valor) & !empty($tipo)){

            include_once '../../../../bd/conecta.php';

            //VERIFICA SE O NOME NÃO É REPETIDO
            $busca_cupom = mysqli_query($conn, "SELECT id FROM cupom WHERE nome = '$nome' AND identificador != '$identificador_cupom'");
            if(mysqli_num_rows($busca_cupom) > 0){                
                ...
                echo "<script>location.href='../../../cupons-edita.php?id=".$identificador_cupom."';</script>";
                ...
                //INSERE NO BANCO
                mysqli_query($conn, "UPDATE cupom SET nome = '$nome', quantidade = '$quantidade', validade = '$validade', valor = '$valor', tipo = '$tipo' WHERE identificador = '$identificador_cupom'");
