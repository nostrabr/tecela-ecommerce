...
$("#tagBox").on("add:after", function ( el, text, tagging ) {
    $.ajax({
        url: "modulos/categorias/php/cadastro-tag.php",
        type: "POST",
        data: {"nome": text},
        success: function (data){
            if(data === "SESSAO INVALIDA") {
                window.location.href = 'logout.php'
            } else if(data === 'SESSAO INVALIDA') {
                mensagemAviso('erro', 'Erro inesperado! Se o problema persistir, contate o administrador do sistema.', 3000);
            }
        }
    });
});
$("#tagBox").on("remove:after", function ( el, text, tagging ) {
    $.ajax({
        url: "modulos/categorias/php/exclusao-tag.php",
        type: "POST",
        data: {"nome": text},
        success: function (data){
            if(data === "SESSAO INVALIDA") {
...
if(tags.length > 0){
    $.each(tags, function(i, jsonData){
        $("#tagBox").tagging("add",jsonData);
    });
}
