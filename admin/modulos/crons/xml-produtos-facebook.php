    ...
    if($tem_variacoes & $loja['loja_roupa'] == 1){
        ...
        while($variacao = mysqli_fetch_array($busca_variacoes_produto)){
            ...
            if($total_imagems_adicionais > 0){
                ...
                $gf_product['additional_image_link'] = $gf_product_adicional_images;
            }

            //ATRIBUTOS            
            $gf_product['item_group_ID']           = $produto['produto_sku'];
            $gf_product['id']                      = $produto['produto_sku'].$contador_variacoes;
            $gf_product['title']                   = $produto['produto_nome'];
            $gf_product['description']             = $descricao;
            $gf_product['link']                    = $link;
            $gf_product['image_link']              = $imagem_capa;
            $gf_product['price']                   = $preco;
            $gf_product['google_product_category'] = $produto['categoria_google'];
            $gf_product['brand']                   = $marca;
            $gf_product['condition']               = 'new';    
            $gf_product['is_clothing']             = true;
            $gf_product['inventory']               = $variacao['estoque'];                            
            $gf_product['age_group']               = $produto['produto_idade'];
            $gf_product['gender']                  = $produto['produto_genero'];  
            ...
            $gf_product['availability']               = $estoque;    
            ...
            if($variacao['id_caracteristica_primaria'] != '' & $variacao['id_caracteristica_secundaria'] != ''){        
                ...
                if($caracteristica_primaria['textura'] != '' | $caracteristica_primaria['cor'] != ''){
                    $gf_product['color'] = $caracteristica_primaria['nome'];
                } else {
                    $gf_product['size']  = $caracteristica_primaria['nome']; 
                }           

                if($caracteristica_secundaria['textura'] != '' | $caracteristica_secundaria['cor'] != ''){
                    $gf_product['color'] = $caracteristica_secundaria['nome'];
                } else {
                    $gf_product['size']  = $caracteristica_secundaria['nome']; 
                }       
            ...
            if($produto['produto_promocao'] != '' | $produto['categoria_promocao'] != ''){
                ...
                $produto_desconto    = $preco*$porcentagem_desconto / 100;   
                $produto_preco_venda = $preco-$produto_desconto;     
                $gf_product['sale_price']   = round($produto_preco_venda, 2);
            }
        ...
        //ATRIBUTOS            
        $gf_product['id']                      = $produto['produto_sku'];
        $gf_product['title']                   = $produto['produto_nome'];
        $gf_product['description']             = $descricao;
        $gf_product['link']                    = $link;
        $gf_product['image_link']              = $imagem_capa;
        ...
        $gf_product['brand']                   = $marca;
        $gf_product['condition']               = 'new';    
        $gf_product['inventory']               = $produto['produto_estoque'];   
        ...
        if ($value != "") {
            ...
            if (is_array($product[$key])) {
                ...
                foreach($product[$key] as $key2=>$value2){   
                    $itemNode->appendChild($doc->createElement('additional_image_link'))->appendChild($doc->createTextNode($value2));
                }
...
$doc->save('produtos-facebook.xml');