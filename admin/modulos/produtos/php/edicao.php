...
if(isset($_SESSION["DONO"])){
    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        if(!empty($identificador_produto) & !empty($nome) & !empty($marca)){
            ...
            if($loja['loja_roupa'] == 1){
                ...
                //EDITA O PRODUTO
                mysqli_query($conn, "UPDATE produto SET id_marca = '$marca', id_categoria = '$categoria', nome = '$nome', gtin = '$gtin', mpn = '$mpn', genero = '$genero', idade = '$idade', categoria_google = '$categoria_google', sku = '$sku', preco = '$preco', altura = '$altura', largura = '$largura', comprimento = '$comprimento', peso = '$peso', descricao = '$descricao', palavras_chave = '$palavras_chave', estoque = '$estoque', atributo_primario = '$atributo_primario', atributo_secundario = '$atributo_secundario', relevancia = '$relevancia', data_edicao = NOW() WHERE identificador = '$identificador_produto'");
            
            } else {

                //EDITA O PRODUTO
                mysqli_query($conn, "UPDATE produto SET id_marca = '$marca', id_categoria = '$categoria', nome = '$nome', gtin = '$gtin', mpn = '$mpn', categoria_google = '$categoria_google', sku = '$sku', preco = '$preco', altura = '$altura', largura = '$largura', comprimento = '$comprimento', peso = '$peso', descricao = '$descricao', palavras_chave = '$palavras_chave', estoque = '$estoque', atributo_primario = '$atributo_primario', atributo_secundario = '$atributo_secundario', relevancia = '$relevancia', data_edicao = NOW() WHERE identificador = '$identificador_produto'");
            
            }

            //PEGA O ID DO PRODUTO
            $busca_produto = mysqli_query($conn, "SELECT id FROM produto WHERE identificador = '$identificador_produto'");
            $produto       = mysqli_fetch_array($busca_produto);
            $id_produto    = $produto['id'];
            
            //EDITA AS TAGS DO PRODUTO   
            $tags         = [];
            if(isset($_POST['tags'])){
                $tags     = $_POST['tags']; 
            }  
            $n_tags       = count($tags);
            $tags_produto = mysqli_query($conn, "SELECT id, id_tag FROM produto_tag WHERE id_produto = ".$id_produto);

            if(mysqli_num_rows($tags_produto) > 0){

                //BUSCA TAGS CADASTRADAS E REMOVE DO BANCO SE NÃO ESTÁ NAS TAGS QUE VIERAM DA EDIÇÃO
                $array_tags_produto = [];
                while($tag_produto = mysqli_fetch_array($tags_produto)){
                    $array_tags_produto[] = $tag_produto['id_tag'];
                    if(!in_array($tag_produto['id_tag'], $tags)){ 
                        mysqli_query($conn, "DELETE FROM produto_tag WHERE id_produto = '$id_produto' AND id_tag = '".$tag_produto['id_tag']."'");
                    }
                }

                //INSERE AS QUE NÃO ESTÃO CADASTRADAS
                for($i = 0; $i < $n_tags; $i++){
                    if(!in_array($tags[$i], $array_tags_produto)){ 
            ...
            //EXCLUI AS VARIAÇÕES E CARACTERISTICAS EXISTENTES, E RECADASTRA AS NOVAS
            mysqli_query($conn, "UPDATE produto_variacao SET status = 2 WHERE id_produto = '$id_produto'");
            mysqli_query($conn, "UPDATE produto_caracteristica SET status = 0 WHERE id_produto = '$id_produto'");

            //SE TEM CARACTERISTICAS TEM VARIAÇÕES E CADASTRA
            if($variacoes != ''){
            ...
            if(isset($_POST['imagem'])){
                ...
                //RECEBE AS IMAGENS E INSTANCIA OS ARRAYS E VARIÁVEIS
                $sql_imagens_manter = '';
                ...
                for($j = 0; $j < $n_imagens; $j++){
                    ...
                    if(isset($ordem_capa_nome[3])){
                        array_push($array_nomes, $ordem_capa_nome[2]);
                        array_push($array_ids, substr($ordem_capa_nome[3],0,-1));
                    } else {                        
                ...
                //SELECIONA AS IMAGENS QUE NÃO PODEM SER EXCLUÍDAS E PREENCHE A STRING QUE FINALIZA O SQL DE BUSCA DE IMAGENS A SEREM ESCLUÍDAS, CADASTRA AS NOVAS E ALTERA AS ANTIGAS
                for($y = 0; $y < count($array_ids); $y++){
                    if($array_ids[$y] != ''){
                        $sql_imagens_manter .= "AND id != ".$array_ids[$y]." ";
                        mysqli_query($conn, "UPDATE produto_imagem SET capa = '$array_capas[$y]', ordem = '$array_ordens[$y]' WHERE id = '$array_ids[$y]'");
                    } else {
                        
                        //PEGA A EXTENSÃO DA IMAGEM
                        $extensao = mb_strtolower(pathinfo($array_nomes[$y], PATHINFO_EXTENSION));
                        ...
                        if($extensao == 'png' | $extensao == 'jpg' | $extensao == 'jpeg' | $extensao == 'gif'){
                            ...
                            $identificador_imagem = md5($y.time().$identificador_usuario_session);   
                            ...
                            acertaOrietacaoImagem($diretorio.$array_nomes[$y]);   

                            //MOVE A IMAGEM RENOMEADA PARA A NOVA PASTA
                            copy($diretorio.$array_nomes[$y], "../../../../imagens/produtos/original/".$nome_imagem);
                            ...
                            //INSERE NO BANCO
                            mysqli_query($conn, "INSERT INTO produto_imagem (identificador, id_produto, imagem, capa, ordem) VALUES ('$identificador_imagem','$id_produto','$nome_imagem','".$array_capas[$y]."','".$array_ordens[$y]."')");
                        
                            $id_produto_recem_adicionado = mysqli_insert_id($conn);

                            $sql_imagens_manter .= "AND id != ".$id_produto_recem_adicionado." ";

                        }
                    }
                }
                
                //BUSCA AS IMAGENS QUE DEVEM SER EXCLUÍDAS
                if($sql_imagens_manter != ''){
                    $busca_imagens_para_excluir = mysqli_query($conn, "SELECT id, imagem FROM produto_imagem WHERE id_produto = ".$id_produto." ".$sql_imagens_manter);
                    while($images_excluir = mysqli_fetch_array($busca_imagens_para_excluir)){
                        
                        //REMOVE OS ARQUIVOS
                        unlink("../../../../imagens/produtos/original/".$images_excluir['imagem']);
                        unlink("../../../../imagens/produtos/pequena/".$images_excluir['imagem']);
                        unlink("../../../../imagens/produtos/media/".$images_excluir['imagem']);
                        unlink("../../../../imagens/produtos/grande/".$images_excluir['imagem']);

                        //REMOVE DO BANCO
                        mysqli_query($conn, "DELETE FROM produto_imagem WHERE id = ".$images_excluir['id']);

                    }
                }

            //SE FORAM TODAS EXCLUÍDAS, REMOVE TUDO
            } else {

                $busca_imagens = mysqli_query($conn, "SELECT id, imagem FROM produto_imagem WHERE id_produto = '$id_produto'");
                while($imagem = mysqli_fetch_array($busca_imagens)){
                        
                    //REMOVE OS ARQUIVOS
                    unlink("../../../../imagens/produtos/original/".$imagem['imagem']);
                    unlink("../../../../imagens/produtos/pequena/".$imagem['imagem']);
                    unlink("../../../../imagens/produtos/media/".$imagem['imagem']);
                    unlink("../../../../imagens/produtos/grande/".$imagem['imagem']);

                    //REMOVE DO BANCO
                    mysqli_query($conn, "DELETE FROM produto_imagem WHERE id = ".$imagem['id']);
