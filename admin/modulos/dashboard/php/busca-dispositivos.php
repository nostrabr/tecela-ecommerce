...
if(isset($_SESSION["DONO"])){
    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        if(mb_strlen($data_inicio) == 10 & mb_strlen($data_fim) == 10){
            ...
            $busca_dispositivos = mysqli_query($conn, "
                SELECT dispositivo, COUNT(id) AS total 
                FROM visita
            ...
            while($dispositivo = mysqli_fetch_array($busca_dispositivos)){
                $array_dispositivos[] = array(
                    "dispositivo" => $dispositivo['dispositivo'],
                    "total"       => $dispositivo['total']
                );
            }

            echo json_encode($array_dispositivos);
