...
if(isset($_SESSION["DONO"])){
    ...
    if($_SESSION["DONO"] !== $token_usuario){
        ...
        if(mb_strlen($data_inicio) == 10 & mb_strlen($data_fim) == 10){
            ...
            $array_formas_entrega = [];

            $busca_formas_entrega = mysqli_query($conn, "
                SELECT COUNT(id) AS total, tipo_frete
            ...
            while($forma_entrega = mysqli_fetch_array($busca_formas_entrega)){
                $array_formas_entrega[] = array(
                    "tipo"  => $forma_entrega['tipo_frete'], 
                    "total" => intval($forma_entrega['total'])
                );
            } 

            echo json_encode($array_formas_entrega);
