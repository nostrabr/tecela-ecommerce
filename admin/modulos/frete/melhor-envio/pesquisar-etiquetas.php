...
if(count($orders > 0)){
    ...
    for($i = 0; $i < count($orders); $i++){
        ...
        if($melhor_envio['melhor_envio_ambiente'] == 'S'){
            $url = "https://sandbox.melhorenvio.com.br/api/v2/me/orders/search?q=$order_id";
        } else {
            $url = "https://melhorenvio.com.br/api/v2/me/orders/search?q=$order_id";
        }
        ...
        array_push($array_pesquisas, json_decode($response,true));
            
    }

    $array_geral[] = array(
        "pesquisas"        => $array_pesquisas,
    );

    echo json_encode($array_geral);
